...
def _extract_result_count(payload: object) -> int:
    """ToolMessage などの payload から results 配列の長さを取得する"""
    data = None
        ...
        for block in payload:
            if isinstance(block, dict):
                if "json" in block:
                    data = block.get("json")
                    break
                if block.get("type") == "json" and "content" in block:
...
def compute_summary_stats(result: dict, final_answer: str) -> tuple[int, int, int]:
    """steps / tool_calls / sources を LangGraph から推定する"""
    ...
    if intermediate_steps:
        ...
        for step in intermediate_steps:
            if isinstance(step, (list, tuple)) and len(step) >= 2 and step[1] is not None:
                sources += _extract_result_count(step[1])
        return steps, tool_calls, sources

    # LangGraph v1 create_agent は intermediate_steps を返さないため、messages から推定する
