...
# サンプル文書の読み込み
data_dir = Path(__file__).parent / "../scripts/chapter5/hybrid_sample_data"
docs: list[Document] = []
for i, f in enumerate(sorted(data_dir.glob("hybrid_doc_*.txt"))):
    docs.append(Document(page_content=f.read_text(encoding="utf-8"),
                         metadata={"source": f.name, "id": i + 1}))

# チャンク分割（今回は1文書=1チャンクでも動く）
...
bm25_retriever.k = 8  # 再ランキング用に多めに取る
...
# EnsembleRetrieverでRRF融合（weightsはBM25:0.6, Vector:1.0）
