...
以下は `scripts/chapter5/5-5-1-hybrid-search-rrf.py` の一部を省略しています。全文はスクリプトを参照してください。
...
固有名詞を含む具体クエリでは BM25 が強みを発揮します。ベクトル検索でも 1 位は同じ文書になりますが、2 位と 3 位は異なる結果を示しています。
...
カタカナの「セマンティック」は単語として存在しないため、BM25 は汎用的な「検索」語を多く含む doc02 を上位に上げました。一方ベクトル検索では doc03（意味検索の解説）を 1 位に引き上げ、ベクトル検索の強みが出ました。
...
1 位が BM25 重視では doc02、ベクトル重視では doc03 に入れ替わり、weights パラメータで結果が変化する様子が分かります。
