...
「埋め込み検索のメリット」という抽象的な質問では、BM25はキーワードが多く含まれる概論（03）や関連キーワードを含む文書（05,02）を並べますが、ベクトル検索は「Dense Retrieval やハイブリッドの利点」を具体的に説明している文書（05）を最上位に押し上げ、検索技術の概論（03）、RRF解説（06）を続けて返します。語の一致だけでは拾いきれないニュアンスを embedding が補完している例です。
