...
実際にハイブリッド検索を実装してみましょう。LangChain の EnsembleRetriever（アンサンブルレトリーバー）を使えば、RRF（スコアリング）の計算も自動でやってくれます。プログラムは `scripts/chapter5/5-5-1-hybrid-search-rrf.py` を参照してください。
...
# ドキュメントの読み込みと分割
loader = TextLoader("./data/ch05/documents.md")
docs = loader.load()
...
# EnsembleRetriever でハイブリッド検索を実装
# weights は各 Retriever の重み付け（デフォルトは均等）
hybrid_retriever = EnsembleRetriever(
    retrievers=[bm25_retriever, dense_retriever],
    weights=[0.6, 1.0]  # BM25 を少し抑えめ、ベクトルを重視
)
...
for i, doc in enumerate(results[:4]):
    print(f"結果 {i+1} {doc.metadata.get('source', '')} 内容: {doc.page_content[:100]}...")
```

EnsembleRetriever はそのまま LCEL パイプライン（「4章 4.3.4Chain」参照）の Retriever として使用できるため、既存の RAG システムに簡単に組み込むことができます。  
ハイブリッド検索の全体像が視覚的にわかりやすい図では、ユーザーのクエリが 2 つの異なる検索手法（キーワードベースの BM25 と意味ベースのベクトル検索）で並列処理され、それぞれの検索結果が RRF（Reciprocal Rank Fusion）アルゴリズムによって統合される様子を表しています。

図の各コンポーネントの役割は以下のとおりです。

- ユーザークエリ：検索したい質問や文章が入力される
- BM25 Retriever：キーワードマッチングによる検索を実行し、単語の出現頻度と文章内での重要度に基づいて関連文書を取得
- Vector Retriever：クエリをベクトル化し、意味的に類似した文書をコサイン類似度などで検索
- RRF（Reciprocal Rank Fusion）：両方の検索結果を順位に基づいてスコアリングし、最終的な統合結果を生成
- Fused Result（統合された検索結果）：両手法の長所を活かした、より精度の高い検索結果が得られる
